...
namespace EcomailDeps\PHPStan\PhpDocParser\Parser;

use EcomailDeps\PHPStan\PhpDocParser\Ast;
use EcomailDeps\PHPStan\PhpDocParser\Ast\Type\IdentifierTypeNode;
...
class PhpDocParser
{
    ...
    public function parse(TokenIterator $tokens) : Ast\PhpDoc\PhpDocNode
    {
        ...
        } catch (\EcomailDeps\PHPStan\PhpDocParser\Parser\ParserException $e) {
            $name = '';
            if (\count($children) > 0) {
    ...
    private function parseExtendsTagValue(string $tagName, TokenIterator $tokens) : Ast\PhpDoc\PhpDocTagValueNode
    {
        ...
        throw new \EcomailDeps\PHPStan\ShouldNotHappenException();
    }
    ...
    private function parseTypeAliasImportTagValue(TokenIterator $tokens) : Ast\PhpDoc\TypeAliasImportTagValueNode
    {
        ...
        if (!$tokens->tryConsumeTokenValue('from')) {
            throw new \EcomailDeps\PHPStan\PhpDocParser\Parser\ParserException($tokens->currentTokenValue(), $tokens->currentTokenType(), $tokens->currentTokenOffset(), Lexer::TOKEN_IDENTIFIER);
        }
