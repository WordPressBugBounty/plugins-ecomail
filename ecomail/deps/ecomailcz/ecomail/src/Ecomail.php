...
class Ecomail
{
     ...
     */
    public function __construct($key, $response = self::JSONArray, $server = 'https://api2.ecomailapp.cz', array $query = array())
    {
        $this->key = $key;
        $this->server = $server;
    ...
    private function send($request, $data, $method, array $query)
    {
        ...
        if (!\curl_errno($ch)) {
            ...
            if ($http_code < 200 || $http_code > 299) {
                return array('error' => $http_code, 'message' => $output);
            }
        }
        \curl_close($ch);
        switch ($this->response) {
            ...
            case self::JSONObject:
                if (\is_array(\json_decode($raw_output, \true))) {
